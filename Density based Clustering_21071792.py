...
def read_csv(file_name):
    ...
    """
    This function is used to read the csv file from the directory and to import the data for the Density clustering.
...
def cluster_plot(title_1, title_2, image_1, image_2):
    """
    'Cluster_plot' function is used to standardize and to normalize the data by removing the mean and making the data to be more accurate.
    ...
    #KNN method
    """
    By using KNN method its easy to find the data points which are close to the chosen point. 
    """
    ...
    plt.figure()
    plt.title(title_2)
    p = sb.scatterplot(data=plot_data, x=x, y=y, hue=dbscan.labels_, legend= "full", palette="deep")
    sb.move_legend(p,"upper right", bbox_to_anchor=(1.10, 1.10), title="CLUSTERS")
    plt.show()
    plt.savefig(image_2, dpi = 300)
    return


#Inoking the function by passing the parameters.
cluster_plot('X', 'Y', 'The distance between the neighbor points ', 'Sales Transactions per Week', "line plot.png", 'scatter plot.png')
